...
namespace Laundry
{
    public partial class UserControlPengeluaran : UserControl
    {
        ...
        private void Tampilkan()
        {
            ...
            {
                DataTable data = Db.Read($"SELECT * FROM tb_pengeluaran join tb_outlet on tb_pengeluaran.id_outlet = tb_outlet.id JOIN tb_user ON tb_pengeluaran.id_user = tb_user.id ORDER BY tb_pengeluaran.id DESC");
            ...
            {
                DataTable data = Db.Read($"SELECT * FROM tb_pengeluaran join tb_outlet on tb_pengeluaran.id_outlet = tb_outlet.id JOIN tb_user ON tb_pengeluaran.id_user = tb_user.idWHERE tb_pengeluaran.id_outlet = {Session.getUserLogged().Rows[0].Field<int>("id_outlet")} ORDER BY tb_pengeluaran.id DESC");
                dataGridViewPengeluaran.AutoGenerateColumns = false;
                dataGridViewPengeluaran.DataSource = data;
            }
            hitungPengeluaran();
        ...
        private void CariData(string keyword)
        {
            ...
            {
                dataGridViewPengeluaran.AutoGenerateColumns = false;
                dataGridViewPengeluaran.DataSource = Db.Read($"SELECT * FROM tb_pengeluaran join tb_outlet on tb_pengeluaran.id_outlet = tb_outlet.id WHERE CONCAT (tb_outlet.nama_outlet, tb_pengeluaran.nama_barang, tb_pengeluaran.tgl, tb_pengeluaran.harga, tb_pengeluaran.keterangan) LIKE '%{keyword}%'");
            }
            else if (Session.getUserLogged().Rows[0].Field<string>("role") != "admin")
            {
                dataGridViewPengeluaran.AutoGenerateColumns = false;
                dataGridViewPengeluaran.DataSource = Db.Read($"SELECT * FROM tb_pengeluaran join tb_outlet on tb_pengeluaran.id_outlet = tb_outlet.id WHERE tb_pengeluaran.id_outlet = {Session.getUserLogged().Rows[0].Field<int>("id_outlet")} AND concat(tb_pengeluaran.nama_barang, tb_pengeluaran.tgl, tb_pengeluaran.harga, tb_pengeluaran.keterangan) LIKE '%{keyword}%' ");
            }
            hitungPengeluaran();
        }
        ...
        private void dataGridViewPengeluaran_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == dataGridViewPengeluaran.Columns["edit"].Index)
            {
                ...
                string total = row.Cells["harga"].Value.ToString();
